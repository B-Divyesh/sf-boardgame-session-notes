...
const routes = new Map([['/', 'index.html'], ['/index.html', 'index.html'], ['/demo', 'demo/index.html'], ['/privacy', 'privacy/index.html'], ['/terms', 'terms/index.html'], ['/404', '404.html'], ['/404/', '404.html']]);

createServer(async (request, response) => {
  const pathname = decodeURIComponent(new URL(request.url || '/', 'http://localhost').pathname);
  if (['/demo/', '/privacy/', '/terms/'].includes(pathname)) {
    response.writeHead(301, { Location: pathname.slice(0, -1), 'Cache-Control': 'no-store' });
    response.end();
    return;
  }
  const dynamicRoute = pathname.startsWith('/session/') || pathname.startsWith('/demo/session/');
  const mapped = dynamicRoute ? 'index.html' : routes.get(pathname);
