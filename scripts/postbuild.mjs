...
const buildId = process.env.VITE_BUILD_ID || execFileSync('git', ['rev-parse', '--short=12', 'HEAD'], { encoding: 'utf8' }).trim();
for (const asset of (await readdir('dist/assets')).filter((name) => name.endsWith('.js'))) {
  const path = `dist/assets/${asset}`;
  await writeFile(path, (await readFile(path, 'utf8')).replaceAll('__BUILD_ID__', buildId));
}
