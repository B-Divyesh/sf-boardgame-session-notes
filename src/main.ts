...
import { isGameSession, newSession, sampleSession, uid, type AppBackup, type GameSession, type SessionEvent } from './types';
...
document.querySelector<HTMLElement>('#build-id')!.textContent = `build ${__BUILD_ID__}`;
...
const normalizedPath = (path: string): string => path.length > 1 && path.endsWith('/') ? path.slice(0, -1) : path;
const isDemoPath = (path: string): boolean => normalizedPath(path) === '/demo' || normalizedPath(path).startsWith('/demo/session/');
const sessionIdFromPath = (path: string, demo: boolean): string | null => {
  const match = normalizedPath(path).match(demo ? /^\/demo\/session\/([^/]+)$/ : /^\/session\/([^/]+)$/);
  if (!match) return null;
  try { return decodeURIComponent(match[1]); } catch { return null; }
};
const wantsDemoAt = (url: URL): boolean => isDemoPath(url.pathname) || url.searchParams.get('demo') === '1';
...
const notePath = (id: string): string => `${demoMode ? '/demo' : ''}/session/${encodeURIComponent(id)}`;
const listPath = (): string => demoMode ? '/demo' : '/';
const draftKey = (): string => `${demoMode ? 'demo:' : ''}${realDatabaseName}:pending-draft`;
...
function pageTitle(label: string): string {
  if (label === 'Home') return 'Boardgame Session Notes — record one boardgame session';
  const maximumLabelLength = 33;
  const shortLabel = label.length > maximumLabelLength ? `${label.slice(0, maximumLabelLength - 1)}…` : label;
...
function setPage(label: string, description: string, path: string): void {
  ...
  document.head.querySelector<HTMLLinkElement>('link[rel="canonical"]')?.setAttribute('href', `${siteUrl}${path}`);
}

function announceRoute(message: string, focus = false): void {
  routeAnnouncement.textContent = '';
  requestAnimationFrame(() => { routeAnnouncement.textContent = message; });
  if (focus) requestAnimationFrame(() => main.querySelector<HTMLElement>('h1')?.focus());
}

function showToast(message: string, action?: { label: string; run: () => void | Promise<void> }): void {
  const toast = document.createElement('div');
  toast.className = 'toast';
  toast.innerHTML = `<span>${html(message)}</span>${action ? `<button type="button">${html(action.label)}</button>` : ''}`;
  if (action) toast.querySelector('button')?.addEventListener('click', () => { void action.run(); toast.remove(); });
  toastRegion.replaceChildren(toast);
  window.setTimeout(() => toast.remove(), action ? 8000 : 4500);
}

function updateConnection(online = navigator.onLine): void {
  connectionStatus.textContent = online ? 'Online · saved in this browser' : 'Offline · saved in this browser';
...
function archiveRows(filtered: GameSession[]): string {
  if (!sessions.length) return '<div class="empty-state"><div class="empty-glyph" aria-hidden="true"><span></span><span></span><span></span></div><h2>No session notes yet</h2><p>Start with a game title. Add players, setup notes, rulings, and scores as you play.</p><button class="secondary-action" id="empty-new-session" type="button">Create a session note</button></div>';
  if (!filtered.length) return '<div class="empty-state compact"><h2>No matching session notes</h2><p>Try another game title, player, or location.</p></div>';
  return `<ul class="session-list">${filtered.map((session) => {
    const players = session.participants.filter((person) => person.name).map((person) => person.name).join(', ') || 'No players named';
...
function renderHome(): void {
  current = null;
  setPage('Home', 'Record setup, rulings, score changes, and outcomes for one boardgame session.', '/');
  const query = search.trim().toLowerCase();
  const filtered = sessions.filter((session) => !query || [session.title, session.location, ...session.participants.map((person) => person.name)].join(' ').toLowerCase().includes(query));
...
function participantRows(session: GameSession): string {
  return session.participants.map((person, index) => `<div class="participant-row" data-person="${html(person.id)}"><span class="player-number" aria-hidden="true">${index + 1}</span><label><span>Player ${index + 1}</span><input data-person-field="name" value="${html(person.name)}" autocomplete="name" placeholder="Name"></label><label class="score-field"><span>Final score</span><input data-person-field="score" value="${html(person.score)}" inputmode="decimal" placeholder="—"></label>${session.participants.length > 1 ? `<button class="icon-button" type="button" data-remove-person="${html(person.id)}" aria-label="Remove player ${index + 1}">×</button>` : ''}</div>`).join('');
}
...
function renderEditor(moveFocus = false, focusSelector?: string): void {
  ...
  const rootDemo = demoMode && (normalizedPath(location.pathname) === '/demo' || new URLSearchParams(location.search).get('demo') === '1');
  const path = rootDemo ? '/demo' : notePath(session.id);
  setPage(demoMode ? 'Demo' : session.title || 'New session note', demoMode ? 'Try a filled sample boardgame session note.' : 'Edit a session note stored in this browser.', path);
...
function syncEditor(): void {
  ...
  document.querySelectorAll<HTMLElement>('[data-person]').forEach((row) => {
    ...
    row.querySelectorAll<HTMLInputElement>('[data-person-field]').forEach((input) => {
      if (person && input.dataset.personField === 'name') person.name = input.value;
      if (person && input.dataset.personField === 'score') person.score = input.value;
    });
  });
}
...
function removeStoredDraft(snapshot?: GameSession): void {
  ...
  try {
    const stored = JSON.parse(localStorage.getItem(draftKey()) || 'null') as unknown;
    if (isGameSession(stored) && stored.id === snapshot.id && stored.updatedAt === snapshot.updatedAt) localStorage.removeItem(draftKey());
  } catch {
...
function currentSnapshot(): GameSession | null {
  ...
  current.updatedAt = new Date(Math.max(Date.now(), Number.isNaN(previousUpdate) ? 0 : previousUpdate + 1)).toISOString();
  ...
  saveChain = saveChain.catch(() => undefined).then(async () => {
    try {
      ...
      if (current?.id === snapshot.id) {
        const status = document.querySelector('#save-status');
        if (status) status.textContent = navigator.onLine ? 'Saved in this browser' : 'Saved offline in this browser';
      }
    } catch {
      const status = document.querySelector('#save-status');
...
async function saveCurrent(immediate = false): Promise<void> {
  ...
  saveTimer = window.setTimeout(() => { saveTimer = undefined; void persistSnapshot(snapshot); }, 450);
}

async function flushCurrent(): Promise<void> {
  if (saveTimer) window.clearTimeout(saveTimer);
  saveTimer = undefined;
  if (current && document.querySelector('#editor-title')) await saveCurrent(true);
...
async function recoverStoredDraft(): Promise<void> {
  ...
  try {
    ...
    if (!stored || draft.updatedAt >= stored.updatedAt) {
      ...
      sessions = [draft, ...sessions.filter((session) => session.id !== draft.id)].sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
    }
...
function updateEditorHeading(): void {
  ...
  if (!demoMode) setPage(current.title || 'New session note', 'Edit a session note stored in this browser.', notePath(current.id));
}
...
function legalPage(type: 'privacy' | 'terms'): void {
  ...
  setPage(label, privacy ? 'How Boardgame Session Notes stores session notes in your browser.' : 'Terms for Boardgame Session Notes.', `/${type}`);
...
function notFound(message = 'This address does not open a Boardgame Session Notes page.'): void {
  current = null;
  setPage('Page not found', 'This Boardgame Session Notes page does not exist.', '/404');
...
function renderDemo(): void {
  if (!sessions.length) { void seedDemo().then(renderDemo); return; }
...
function route(path = location.pathname, moveFocus = false): void {
  ...
  if (sessionId) {
    if (!openSession(sessionId, false, moveFocus)) notFound('This session note was not found in this browser.');
  } else if (demoMode && (normalized === '/demo' || new URLSearchParams(location.search).get('demo') === '1')) {
    renderDemo();
...
function openPrintReceipt(session: GameSession): void {
  ...
  const receiptUrl = URL.createObjectURL(new Blob([markup], { type: 'text/html' }));
...
main.addEventListener('input', (event) => {
  ...
  if (target.id === 'session-search') {
    search = target.value;
    const filtered = sessions.filter((session) => [session.title, session.location, ...session.participants.map((person) => person.name)].join(' ').toLowerCase().includes(search.toLowerCase()));
...
main.addEventListener('change', async (event) => {
  ...
  if (target.id === 'setup-photo' && current && target instanceof HTMLInputElement && target.files?.[0]) {
    try {
      ...
      showToast(error instanceof Error ? error.message : 'The photo could not be added.');
  ...
  if (target.id === 'import-backup' && target instanceof HTMLInputElement && target.files?.[0]) {
    ...
    try {
      ...
      if (status) status.textContent = `Restored ${backup.sessions.length} session notes.`;
      showToast('Backup file restored.');
    } catch (error) {
      if (status) status.textContent = error instanceof Error ? error.message : 'The backup file could not be read.';
    }
  }
});
...
main.addEventListener('submit', async (event) => {
  ...
  if (form.id === 'add-event-form' && current) {
    ...
    if (!note) return;
    current.events.push({ id: uid(), time: form.querySelector<HTMLInputElement>('#event-time')!.value, kind: form.querySelector<HTMLSelectElement>('#event-kind')!.value as SessionEvent['kind'], note });
    await saveCurrent(true);
...
main.addEventListener('click', async (event) => {
  const button = (event.target as HTMLElement).closest<HTMLElement>('button, [data-open-session]');
  if (!button) return;
  if (button.id === 'reload-app') location.reload();
  else if (button.id === 'new-session' || button.id === 'empty-new-session') await createSession();
    ...
    await saveCurrent(true);
    renderEditor(false, `[data-person="${current.participants.at(-1)!.id}"] input`);
  } else if (button.dataset.removePerson && current) {
    syncEditor();
    current.participants = current.participants.filter((person) => person.id !== button.dataset.removePerson);
    await saveCurrent(true);
    renderEditor(false);
  } else if (button.dataset.removeRule && current) {
    syncEditor();
    current.houseRules.splice(Number(button.dataset.removeRule), 1);
    await saveCurrent(true);
    renderEditor(false);
  } else if (button.dataset.removeEvent && current) {
    syncEditor();
    current.events = current.events.filter((item) => item.id !== button.dataset.removeEvent);
    await saveCurrent(true);
    renderEditor(false);
  ...
  } else if ((button.id === 'toggle-complete' || button.id === 'finish-session') && current) {
    syncEditor();
    ...
    current.complete = button.id === 'finish-session' ? true : !current.complete;
    await saveCurrent(true);
    renderEditor(false);
    showToast(current.complete ? 'Session note marked complete.' : 'Session note reopened.');
    ...
    if (!confirm(`Delete “${label}” from this browser? You can undo for a few seconds.`)) return;
    ...
    downloadText(`session-notes-backup-${new Date().toISOString().slice(0, 10)}.json`, JSON.stringify(createBackup(sessions, snippets), null, 2), 'application/json');
...
document.querySelector('#open-data-tools')?.addEventListener('click', () => { void openDataDialog(); });
  ...
  try {
    ...
    registration.addEventListener('updatefound', () => {
      ...
      worker?.addEventListener('statechange', () => {
        if (worker.state === 'installed' && navigator.serviceWorker.controller) showToast('A new app version is ready.', { label: 'Reload', run: () => location.reload() });
      });
    });
  } catch {
...
async function initialize(): Promise<void> {
  ...
  main.innerHTML = '<div class="loading-state"><span aria-hidden="true"></span><p>Opening session notes…</p></div>';
  try {
    ...
    const directSession = sessionIdFromPath(location.pathname, demoMode) !== null;
