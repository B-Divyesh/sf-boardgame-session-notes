...
Generated with the factory image model (`factory-image`, Azure OpenAI image generation) on 2026-08-27. The final prompt is stored at `assets/src/session-map.json`; source PNG is retained at `assets/src/session-map.png`. `assets/src/session-map.provenance.json` records SHA-256 values for the source and every shipped derivative. Generated imagery is original to this product and disclosed in the footer.
